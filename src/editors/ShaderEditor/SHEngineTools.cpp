...
public:
    virtual void Parse(CSHEngineTools *owner, DWORD type, LPCSTR key, LPVOID data)
    {
        switch (type)
        {
            case xrPID_MATRIX: owner->CollapseMatrix((LPSTR)data);
                break;
            case xrPID_CONSTANT: owner->CollapseConstant((LPSTR)data);
                break;
        };
    }
};
...
public:
    virtual void Parse(CSHEngineTools *owner, DWORD type, LPCSTR key, LPVOID data)
    {
        switch (type)
        {
            case xrPID_MATRIX: owner->UpdateMatrixRefs((LPSTR)data);
                break;
            case xrPID_CONSTANT: owner->UpdateConstantRefs((LPSTR)data);
                break;
        };
    }
};
...
public:
    virtual void Parse(CSHEngineTools *owner, DWORD type, LPCSTR key, LPVOID data)
    {
        switch (type)
        {
            case xrPID_MATRIX: owner->RemoveMatrix((LPSTR)data);
                break;
            case xrPID_CONSTANT: owner->RemoveConstant((LPSTR)data);
                break;
        };
    }
};
...
bool CSHEngineTools::OnPreviewObjectRefChange(PropValue *sender, u32 &new_val)
{
    ...
    switch (new_val)
    {
        case pvoPlane: fn = "editor\\ShaderTest_Plane";
            break;
        case pvoBox: fn = "editor\\ShaderTest_Box";
            break;
        case pvoSphere: fn = "editor\\ShaderTest_Sphere";
            break;
        case pvoTeapot: fn = "editor\\ShaderTest_Teapot";
            break;
        case pvoCustom: fn = m_PreviewObject ? m_PreviewObject->GetName() : "";
...
LPCSTR CSHEngineTools::GenerateMatrixName(LPSTR name)
{
    int cnt = 0;
    do
        sprintf(name, "%04x", cnt++); while (FindMatrix(name));
    return name;
}
...
LPCSTR CSHEngineTools::GenerateConstantName(LPSTR name)
{
    int cnt = 0;
    do
        sprintf(name, "%04x", cnt++); while (FindConstant(name));
    return name;
}
...
LPCSTR CSHEngineTools::AppendItem(LPCSTR folder_name, LPCSTR parent_name)
{
    ...
    {
        LPCSTR M = 0;
        if (!TfrmChoseItem::SelectItem(smCustom, M, 1, 0, fastdelegate::bind<TOnChooseFillItems>(this, &CSHEngineTools::FillChooseTemplate))||!M)
            return 0;
        for (TemplateIt it = m_TemplatePalette.begin(); it!=m_TemplatePalette.end(); it++)
    ...
    while (!data.eof())
    {
        ...
        switch (type)
        {
            ...
            case xrPID_CONSTANT:
                ...
                break;
            case xrPID_TEXTURE: sz = sizeof(string64);
                break;
            case xrPID_INTEGER: sz = sizeof(xrP_Integer);
                break;
            case xrPID_FLOAT: sz = sizeof(xrP_Float);
                break;
            case xrPID_BOOL: sz = sizeof(xrP_BOOL);
                break;
            case xrPID_TOKEN: sz = sizeof(xrP_TOKEN)+sizeof(xrP_TOKEN::Item)*(((xrP_TOKEN*)data.pointer())->Count);
                break;
...
void CSHEngineTools::ParseBlender(IBlender *B, CParseBlender &P)
{
    ...
    while (!data.eof())
    {
        ...
        {
            case xrPID_MARKER: break;
            case xrPID_MATRIX: sz = sizeof(string64);
                break;
            case xrPID_CONSTANT: sz = sizeof(string64);
                break;
            case xrPID_TEXTURE: sz = sizeof(string64);
                break;
            case xrPID_INTEGER: sz = sizeof(xrP_Integer);
