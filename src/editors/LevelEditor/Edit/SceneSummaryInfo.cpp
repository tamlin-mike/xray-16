...
void SSceneSummary::STextureInfo::OnHighlightClick(ButtonValue *sender, bool &bDataModified, bool &bSafe)
{
    ...
    switch (V->btn_num)
    {
        case 0: Scene->HighlightTexture((LPCSTR)sender->tag, false, info.width, info.height, false);
            break;
        case 1: Scene->HighlightTexture((LPCSTR)sender->tag, true, info.width, info.height, false);
            break;
        case 2: Scene->HighlightTexture((LPCSTR)sender->tag, true, info.width, info.height, true);
            break;
        case 3: ExecCommand(COMMAND_CLEAR_DEBUG_DRAW);
            break;
    }
    bDataModified = false;
    bSafe = false;
...
void SSceneSummary::OnHighlightClick(ButtonValue *V, bool &bDataModified, bool &bSafe)
{
    ...
    switch (V->btn_num)
    {
        case 0:
        {
            ...
            break;
        case 1: ExecCommand(COMMAND_CLEAR_DEBUG_DRAW);
            break;
    }
    bDataModified = false;
    bSafe = false;
