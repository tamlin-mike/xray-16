...
#include "editors/xrEProps/ItemDialog.h"
...
bool CParticleTool::IfModified()
{
    if (m_bModified)
    {
        ...
        switch (mr)
        {
            case mrYes: if (!ExecCommand(COMMAND_SAVE))
                    return false; else
                    m_bModified = FALSE;
                break;
...
void CParticleTool::ZoomObject(BOOL bSelOnly)
{
    ...
    else
    {
        ...
        {
            case emNone: break;
            case emEffect: box.set(m_EditPE->vis.box);
                break;
            case emGroup: box.set(m_EditPG->vis.box);
                break;
            default: THROW;
        }
        if (box.is_valid())
...
{
    LPCSTR fn;
    if (!TfrmChoseItem::SelectItem(smObject, fn, 1, m_EditObject ? m_EditObject->GetName() : 0))
        return;
    Lib.RemoveEditObject(m_EditObject);
...
void CParticleTool::RealApplyParent()
{
    ...
    {
        case emNone: break;
        case emEffect: m_EditPE->UpdateParent(m_Transform, m_Vel, m_Flags.is(flSetXFORM));
            break;
        case emGroup: m_EditPG->UpdateParent(m_Transform, m_Vel, m_Flags.is(flSetXFORM));
            break;
        default: THROW;
    }
