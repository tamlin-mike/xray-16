...
bool get_float(LPCSTR a, var &val)
{
...
bool get_bool(LPCSTR a, var &val)
{
...
bool get_string(LPCSTR src, var &val)
{
    string128 v;
    _GetItem(src, 1, v, '"');
    val = v;
...
int get_token_ID(xr_token *token_list, LPCSTR name)
{
...
bool get_domain_type(LPCSTR src, var &val)
{
...
void ErrMsg(LPCSTR ps, int c, int l, int v, LPCSTR line)
{
    switch (c)
    {
        case BAD_COMMAND: ELog.Msg(mtError, "[Error] '%s'(%d): Unrecognized command: '%s'", ps, l+1, line);
            break;
        case FEW_PARAMS: ELog.Msg(mtError, "[Error] '%s': Too few parameters in call to '%s'", ps, line);
            break;
        case MANY_PARAMS: ELog.Msg(mtError, "[Error] '%s': Extra parameters in call to '%s'", ps, line);
            break;
        case BAD_PARAM: ELog.Msg(mtError, "[Error] '%s'(%d): Bad parameter '%d' - '%s'", ps, l+1, v, line);
            break;
        case STA_MISSING: ELog.Msg(mtError, "[Error] '%s'(%d): Statement missing ';'", ps, l+1, line);
            break;
    }
}
...
public:
    ...
    bool LoadTemplate(LPCSTR line)
    {
        ...
        for (int k = 0; k<p_cnt; k++)
        {
            ...
            if (!P.s_data.IsEmpty())
            {
                switch (P.type)
                {
                    case ptDomain: bRes = get_domain_type(P.s_data.c_str(), P.data);
                        break;
                    case ptBOOL: bRes = get_bool(P.s_data.c_str(), P.data);
                        break;
                    case ptFloat: bRes = get_float(P.s_data.c_str(), P.data);
                        break;
                    case ptInt:
                    case ptWORD:
                    case ptDWORD: bRes = get_int(P.s_data.c_str(), P.data);
                        break;
                    case ptString: bRes = get_string(P.s_data.c_str(), P.data);
                        break;
                }
            }
            else
    ...
    bool Parse(int l, LPCSTR line)
    {
        ...
        for (int k = 0; k<p_cnt; k++)
        {
            ...
            switch (P.type)
            {
                case ptDomain: bRes = get_domain_type(pm.c_str(), P.data);
                    break;
                case ptBOOL: bRes = get_bool(pm.c_str(), P.data);
                    break;
                case ptFloat: bRes = get_float(pm.c_str(), P.data);
                    break;
                case ptInt:
                case ptWORD:
                case ptDWORD: bRes = get_int(pm.c_str(), P.data);
                    break;
                case ptString: bRes = get_string(pm.c_str(), P.data);
                    break;
            }
            if (!bRes)
            {
