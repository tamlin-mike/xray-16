...
void LevelGraphDebugRender::DrawStalkers(int vid)
{
	...
	if (showText)
    {
		...
		for (auto &pair : objects.objects())
        {
            ...
            const float &walked_distance = path.size()<2 ?
                0 : stalker->brain().movement().detail().walked_distance();
...
void LevelGraphDebugRender::DrawObjects(int vid)
{
	...
	if (showText)
    {
		...
		for (auto &pair : objects.objects())
        {
			...
			const float walkedDistance = path.size()<2 ?
                0 : monster->brain().movement().detail().walked_distance();
...
void LevelGraphDebugRender::DrawNodes()
{
    ...
    {
        it = std::lower_bound(levelGraph->begin(), levelGraph->end(),
            levelGraph->vertex_position(minPos).xz(), CLevelGraph::vertex::predicate2);
    }
	else
        it = levelGraph->begin();
	if (levelGraph->valid_vertex_position(maxPos))
    {
        end = std::upper_bound(levelGraph->begin(), levelGraph->end(),
            levelGraph->vertex_position(maxPos).xz(), CLevelGraph::vertex::predicate);
