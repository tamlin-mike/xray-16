...
public:
    ...
    IC SelfRef set(SelfCRef b) { vMin.set(b.vMin); vMax.set(b.vMax); return *this; };
    IC SelfRef setb(const Tvector& center, const Tvector& dim) { vMin.sub(center, dim); vMax.add(center, dim); return *this; }

    IC SelfRef set_zero() { vMin.set(0, 0, 0); vMax.set(0, 0, 0); return *this; };
    IC SelfRef identity() { vMin.set(-0.5, -0.5, -0.5); vMax.set(0.5, 0.5, 0.5); return *this; };
    IC SelfRef invalidate() { vMin.set(type_max(T), type_max(T), type_max(T)); vMax.set(type_min(T), type_min(T), type_min(T)); return *this; }

    IC SelfRef shrink(T s) { vMin.add(s); vMax.sub(s); return *this; };
    IC SelfRef shrink(const Tvector& s) { vMin.add(s); vMax.sub(s); return *this; };
    IC SelfRef grow(T s) { vMin.sub(s); vMax.add(s); return *this; };
    IC SelfRef grow(const Tvector& s) { vMin.sub(s); vMax.add(s); return *this; };

    IC SelfRef add(const Tvector& p) { vMin.add(p); vMax.add(p); return *this; };
    IC SelfRef sub(const Tvector& p) { vMin.sub(p); vMax.sub(p); return *this; };
    IC SelfRef offset(const Tvector& p) { vMin.add(p); vMax.add(p); return *this; };
    IC SelfRef add(SelfCRef b, const Tvector& p) { vMin.add(b.vMin, p); vMax.add(b.vMax, p); return *this; };
    ...
    ICF SelfRef modify(const Tvector& p) { vMin.min(p); vMax.max(p); return *this; }
    ICF SelfRef modify(T x, T y, T z) { _vector3<T> tmp = {x, y, z}; return modify(tmp); }
    IC SelfRef merge(SelfCRef b) { modify(b.vMin); modify(b.vMax); return *this; };
    ...
    ICF SelfRef xform(SelfCRef B, const Tmatrix& m)
    {
        ...
        if (negative(vx.x)) vMin.x += vx.x;
        else vMax.x += vx.x;
        if (negative(vx.y)) vMin.y += vx.y;
        else vMax.y += vx.y;
        if (negative(vx.z)) vMin.z += vx.z;
        else vMax.z += vx.z;
        if (negative(vy.x)) vMin.x += vy.x;
        else vMax.x += vy.x;
        if (negative(vy.y)) vMin.y += vy.y;
        else vMax.y += vy.y;
        if (negative(vy.z)) vMin.z += vy.z;
        else vMax.z += vy.z;
        if (negative(vz.x)) vMin.x += vz.x;
        else vMax.x += vz.x;
        if (negative(vz.y)) vMin.y += vz.y;
        else vMax.y += vz.y;
        if (negative(vz.z)) vMin.z += vz.z;
        else vMax.z += vz.z;
        return *this;
    }
...
template <class T>
bool _valid(const _box3<T>& c) { return _valid(c.vMin) && _valid(c.vMax); }
