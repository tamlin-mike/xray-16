...
// res_ptr < res_ptr
// res_ptr > res_ptr
// XXX: TODO: Make these comparison operators complete (if they are even required). Missing <= and >=.
template<class T>
IC bool operator == (ref_smem<T> const& a, ref_smem<T> const& b) { return a._get() == b._get(); }
template<class T>
IC bool operator != (ref_smem<T> const& a, ref_smem<T> const& b) { return !(a == b); }
template<class T>
IC bool operator < (ref_smem<T> const& a, ref_smem<T> const& b) { return a._get() < b._get(); }
template<class T>
IC bool operator > (ref_smem<T> const& a, ref_smem<T> const& b) { return a._get() > b._get(); }
